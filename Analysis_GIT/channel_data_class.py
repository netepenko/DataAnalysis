...
class channel_data():
    ...
    def __init__(self, shot, channel):
        ...
        # read parameters from Shot_List table in databse
        try:
            (self.par['exp_dir'], self.par['exp_file']) = db.retrieve(
                    'Folder, File_Name', 'Shot_List', 'Shot = ' + str(shot))
        except:
            print "Given shot is not found in the shot list!"
            return
        ...
        # them from another another channel or even another shot
        ...
        try:
            ...
            shot_cp = self.par['shot']   # copy param from previous shot
            ...
            try:
                ...
                print "Couldn't copy paramateres from previous shot!"
                return
        ...
        (decay_time, rise_time, position) = db.retrieve(
                'decay_time, rise_time, position', 'Peak_Sampling', wheredb)
        ...
        (self.par['n_samp'], self.par['n_max'], self.par['n_below'],
         self.par['n_above']) = db.retrieve(
                 'n_samp, n_max, n_below, n_above', 'Peak_Sampling', wheredb)
        ...
        ydata = f[data_root + 'y-axis/data_vector/data'].value.astype(
                'int16')[tds]
...
# (skipping some data points according to scale)
...
channel_data.peak_shape = peak_sampling.peak_shape
channel_data.selec_peak = peak_sampling.select_peak
