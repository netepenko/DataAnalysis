...
class digi_data:
    ...
    def load_NI_data(self, **kwargs):
        ...
        # check time limits
        self.tmin = max(tall[0], self.tmin)
        self.tmax = min(tall[-1], self.tmax)
        # select time range
        ...
        self.i_max = i_max
        # store data
        self.td = tall[i_min:i_max]  # time data (microseconds) in analysis interval
        self.Vps = V[i_min:i_max]  # voltage data
    ...
    def load_gage_data(self, **kwargs):
        ...
        # store data
        td = tall[i_min:i_max]  # time data (microseconds) in analysis interval
        Vps = ydata[i_min:i_max]  # voltage data
...
# (skipping some data points according to maximum allowed points on plot)
    def plot_raw(self, xmin=None, xmax=None, **kwargs):
        ...
        if xmin and xmax:
            ...
            interval = np.where((self.dtmin < t) & (t < self.dtmax))
            t = t[interval]
            V = V[interval]            
